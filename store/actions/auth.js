...
export const login = (email, password) => {
  return async dispatch => {
    ...
    const response = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBeeIWtGxW70vMoeHAauOXTDmMcNEfABsI', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    dispatch(authenticate(
                          ...
                          resData.idToken, 
                          parseInt(resData.expiresIn ) * 1000));
    const expireationData = new Date(new Date().getTime() + parseInt(resData.expiresIn) * 1000);
    saveDataToStorage(resData.idToken, resData.localId, expireationData)
  };
};
